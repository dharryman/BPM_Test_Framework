...
def mock_get_device_ID():
    return "Libera BPM with the Epics ID \"TS-DI-EBPM-00:\" and the MAC Address \"00:d0:50:31:03:b9\""
...
class ExpectedDataTest(unittest.TestCase):
    ...
    @patch("BPMDevice.Libera_BPMDevice._read_epics_pv")
    @patch("BPMDevice.Libera_BPMDevice.get_device_ID")
    def setUp(self, device_mock, epics_mock):
        # Stuff you run before each test
        self.BPM_test_inst = BPMDevice.Libera_BPMDevice(0)
    ...
    @patch("BPMDevice.Libera_BPMDevice.get_device_ID", side_effect=mock_get_device_ID)
    ...
    @patch("BPMDevice.Libera_BPMDevice._read_epics_pv", side_effect=mocked_BPM_replies)
